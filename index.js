...
canvas.height = 576;
const doorMap = []
for(let i=0; i<door.length; i+=878){
    doorMap.push(door.slice(i,878+i))
}
...
const collisionsMap = []
for(let i=0; i<collision1.length; i+=878){
    collisionsMap.push(collision1.slice(i,878+i))
    
}
console.log(collision1.length)
console.log(collisionsMap)
collisionsMap.forEach((row, i) =>{
    row.forEach((symbol, j) => {
        if(symbol === 755 || symbol === 2684355315)
        boundaries.push(
            new Boundary( {
                position: {
...
const player = new Sprite ({
    position:{
        x:canvas.width/2 - (96/4) / 2,  //(192/4) / 2,
        y:canvas.height/2 - 34/2    //68/2
    },
    image: playerImage,
    frames: {
        max:4
